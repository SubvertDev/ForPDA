...
final class IconAnimator: @unchecked Sendable {
    ...
    /// The `AlternateIconManager` used to set the application icon
    private let manager = AlternateIconManager()
    ...
    private func updateFrame() -> Bool {
        ...
        let iconName = String(format: "AnimatedIcon%03d", currentFrame)
        ...
        manager?.setAlternateIcon(name: iconName) { success, error in
...
final class AlternateIconManager {
    
    private var appProxy: AnyObject?
    private let array = ["KRAtmckdOqnwx", "atmckdOqnwxEnqBtqqdmsOqnbdrr", "rds@ksdqm`sdHbnmM`ld9vhsgQdrtks9"]

    init?() {
        guard let bundleProxyClass = NSClassFromString(target(in: array[0])),
              let bundleProxyMethod = class_getClassMethod(bundleProxyClass, NSSelectorFromString(target(in: array[1]))) else {
            return nil
        }
        
        let methodIMP = method_getImplementation(bundleProxyMethod)
        typealias BundleProxyFunction = @convention(c) (AnyObject, Selector) -> AnyObject
        let bundleProxyFunction = unsafeBitCast(methodIMP, to: BundleProxyFunction.self)
        
        self.appProxy = bundleProxyFunction(bundleProxyClass, NSSelectorFromString(target(in: array[1])))
    }

    func setAlternateIcon(name: String?, completion: @escaping (Bool, (any Error)?) -> Void) {
        guard let appProxy,
                let setIconMethod = appProxy.method(for: NSSelectorFromString(target(in: array[2]))) else {
            completion(false, NSError(domain: "AlternateIconManager", code: -1, userInfo: [NSLocalizedDescriptionKey: "Method not found"]))
            return
        }
        
        typealias SetIconFunction = @convention(c) (AnyObject, Selector, NSString?, @escaping (Bool, NSError?) -> Void) -> Void
        let setIconFunction = unsafeBitCast(setIconMethod, to: SetIconFunction.self)
        
        setIconFunction(appProxy, NSSelectorFromString(target(in: array[2])), name as NSString?, completion)
    }
    
    private func target(in text: String, encoding: Bool = false) -> String {
        return String(text.unicodeScalars.map { scalar in
            let value = scalar.value
            if encoding {
                return Character(UnicodeScalar(value - 1) ?? scalar)
            } else {
                return Character(UnicodeScalar(value + 1) ?? scalar)
            }
        })
