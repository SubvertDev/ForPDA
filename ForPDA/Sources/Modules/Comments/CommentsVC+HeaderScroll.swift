...
extension CommentsVC {
    ...
    override func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let topViewCurrentHeightConst = collectionViewScrollDelegate?.currentHeaderHeight
        if let topViewUnwrappedHeight = topViewCurrentHeightConst {
            ...
            if delta < 0,
               topViewUnwrappedHeight < collectionViewScrollDelegate?.topViewHeightConstraintRange.upperBound ?? 0,
               scrollView.contentOffset.y < 0
            {
                dragDirection = .down
                collectionViewScrollDelegate?.innerCollectionViewDidScroll(withDistance: delta)
